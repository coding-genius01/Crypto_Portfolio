...
def my_portfolio():
    ...
    def update_coin():
        cursorObj.execute("UDPATE coin SET symbol=?, price=?, amount=? WHERE id=?", (symbol_update.get(), price_update.get(), amount_update.get(), portfolio_id_update.get()))
        con.commit()
    ...
    def delete_coin():
        cursorObj.execute("DELETE FROM coin WHERE id=?", (portfolio_id_delete.get(),))
        con.commit()
    ...
    for i in range(len(api["data"])):
        for coin in coins:
            if api["data"][i]["symbol"] == coin[1]:
                ...
                potfolio_id = Label(pycrypto, text=coin[0], bg="white", fg="gray", font="Lato 10", padx="5", pady="5", borderwidth=2, relief="groove")
                potfolio_id.grid(row=coin_row, column=0, sticky=N+S+E+W)
    ...
    add_coin = Button(pycrypto, text="Add Coin", bg="#152E54", command=insert_coin, cursor="plus", fg="white", font="Lato 10", padx="5", pady="5", borderwidth=2, relief="groove")
    add_coin.grid(row=coin_row+1, column=4, sticky=N+S+E+W)

    #Update coin
    portfolio_id_update = Entry(pycrypto, borderwidth=2, relief="groove")
    portfolio_id_update.grid(row=coin_row+2, column=1)
    ...
    update_coin_btn = Button(pycrypto, text="Update Coin", bg="#152E54", command=update_coin, cursor="exchange", fg="white", font="Lato 10", padx="5", pady="5", borderwidth=2, relief="groove")
    update_coin_btn.grid(row=coin_row+2, column=4, sticky=N+S+E+W)

    #Delete coin
    portfolio_id_delete = Entry(pycrypto, borderwidth=2, relief="groove")
    portfolio_id_delete.grid(row=coin_row+3, column=0)

    delete_coin_btn = Button(pycrypto, text="Delete Coin", bg="#152E54", command=delete_coin, cursor="target", fg="white", font="Lato 10", padx="5", pady="5", borderwidth=2, relief="groove")
    ...
    totalpl_portfolio.grid(row=coin_row, column=7, sticky=N+S+E+W)

    api = ""

    refresh = Button(pycrypto, text="Refresh", bg="#152E54", command=reset, cursor="exchange", fg="white", font="Lato 10", padx="5", pady="5", borderwidth=2, relief="groove")
    refresh.grid(row=coin_row+1, column=7, sticky=N+S+E+W)

def app_header():
    portfolio_id = Label(pycrypto, text="Portfolio ID", bg="#152E54", fg="white", font="Lato 12 bold", padx="5", pady="5", borderwidth=2, relief="groove")
